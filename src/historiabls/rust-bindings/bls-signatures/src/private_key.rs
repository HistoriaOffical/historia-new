...
impl PrivateKey {
    ...
    #[cfg(feature = "historia_helpers")]
    pub fn generate_historia(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "historia_helpers")]
    pub fn generate_historia_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_historia(rng))
            .collect()
    }
