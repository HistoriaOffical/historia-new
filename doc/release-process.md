...
* Update translations, see [translation_process.md](https://github.com/historia/historia/blob/master/doc/translation_process.md#synchronising-translations).

* Update manpages, see [gen-manpages.sh](https://github.com/historia/historia/blob/master/contrib/devtools/README.md#gen-manpagessh).
...
Check out the source code in the following directory hierarchy.
	...
	git clone https://github.com/historia/guix.sigs.git
	git clone https://github.com/historia/historia-detached-sigs.git
	git clone https://github.com/historia/historia.git

### Historia Core maintainers/release engineers, suggestion for writing release notes
...
Checkout the Historia Core version you'd like to build:
...
Add other builders keys to your gpg keyring, and/or refresh keys: See `../historia/contrib/builder-keys/README.md`.
...
Codesigner only: Sign the macOS binary:
    ...
    transfer historiacore-osx-unsigned.tar.gz to macOS for signing
...
```sh
pushd ~/historiacore-detached-sigs
...
- Upload to the historia.org server:
    1. The contents of each `./historia/guix-build-${VERSION}/output/${HOST}/` directory, except for
       `*-debug*` files.
       ...
       as save storage space *do not upload these to the historia.org server*.

       ```sh
       find guix-build-${VERSION}/output/ -maxdepth 2 -type f -not -name "SHA256SUMS.part" -and -not -name "*debug*" -exec scp {} user@historia.org:/var/www/bin/historia-core-${VERSION} \;
       ```
...
- Announce the release:
  ...
  - Optionally Discord, twitter, reddit /r/Historiapay, ... but this will usually sort out itself
  ...
  - Create a [new GitHub release](https://github.com/historia/historia/releases/new) with a link to the archived release notes.
...
```
xcrun notarytool submit historiacore-{version}-osx.dmg --keychain-profile "AC_PASSWORD" --wait
```
...
After successfully notarizing the .dmg file, extract "Historia-Qt.app" from the .dmg.
To verify that the notarization process was successful, run the following command:

```
spctl -a -vv -t install Historia-Qt.app
```
Replace "Historia-Qt.app" with the path to your .app file. This command checks whether your .app file passes Gatekeeper’s
...
- For `mainnet` -> Take the size of the Historia Core data directory, excluding `/regtest` and `/testnet3` directories.
