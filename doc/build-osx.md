...
1. Clone the Historia Core source code:
    ```shell
...
2.  Build Historia Core:
    ...
    Configure and build the headless Historia Core binaries as well as the GUI (if Qt is found).
...
## `disable-wallet` mode
When the intention is to run only a P2P node without a wallet, Historia Core may be
...
```shell
mkdir -p "/Users/${USER}/Library/Application Support/HistoriaCore"

touch "/Users/${USER}/Library/Application Support/HistoriaCore/historia.conf"

chmod 600 "/Users/${USER}/Library/Application Support/HistoriaCore/historia.conf"
```

The first time you run historiad, it will start downloading the blockchain. This process could take many hours, or even days on slower than average systems.
